...
4. **Webhook**: uncheck **Active** — the action is triggered by your workflow's
   events and uses the App only for authentication, so the App itself needs no
   webhook.
...
Create `.github/PULL_REQUEST_TEMPLATE.md` — GitHub uses it as the repository's
**default** PR body, auto-filling it on open. On open the checked boxes are
converted to `backport:<branch>` labels and this block is removed — after open,
manage backports via those labels. Wrap the block in markers so it strips cleanly.
